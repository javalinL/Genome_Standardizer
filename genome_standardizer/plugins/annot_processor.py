...
def standardize_and_rename(raw_genes, prefix, step=10, longest_only=False):
    processed_genes = {}
    id_mapping = []

    sorted_gids = sorted(
        raw_genes.keys(),
        key=lambda k: (natural_sort_key(raw_genes[k]['scaf']), raw_genes[k]['start'])
    )
    ...
    for gid in tqdm(sorted_gids, desc="         [Progress] Structuring ", unit=" gene", ncols=100, leave=False):
        ...
        for idx, old_mid in enumerate(sorted_mids, 1):
            ...
            new_mrna = {
                'start': m_start,
                'end': m_end,
                'attrs': {'ID': new_mrna_id, 'Parent': new_gene_id, 'Name': new_mrna_id},
                'subfeats': []
            }
