...
def export_all(processed_genes, cds_records, pep_records, id_mapping, prefix, work_dir, logger=None):
    ...
    with open(out_gff, 'w') as f_gff, \
        ...
        for gid, gene in tqdm(processed_genes.items(), desc="         [Progress] Exporting   ", unit=" gene", ncols=100, leave=False):
            ...
            f_gff.write(f"{scaf}\t.\tgene\t{gene['start']}\t{gene['end']}\t.\t{strand}\t.\t{g_attrs}\n")
            ...
            for mid, mrna in gene['mrnas'].items():
                ...
                f_gff.write(f"{scaf}\t.\tmRNA\t{m_start}\t{m_end}\t.\t{strand}\t.\t{m_attrs}\n")

                old_mid = new_to_old_map.get(mid, ".")
                f_bed.write(f"{scaf}\t{m_start}\t{m_end}\t{mid}\t{strand}\t.\t{old_mid}\n")
                ...
                for sub in mrna['subfeats']:
                    ...
                    s_attrs = f"ID={sub['attrs']['ID']};Parent={sub['attrs']['Parent']}"
                    f_gff.write(f"{scaf}\t.\t{s_type}\t{s_start}\t{s_end}\t.\t{strand}\t{s_phase}\t{s_attrs}\n")
